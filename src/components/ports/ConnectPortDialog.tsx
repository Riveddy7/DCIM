...
import type { PortDetails } from '@/lib/database.types';
...
export function ConnectPortDialog({ portA, tenantId, onSuccess, onCancel }: ConnectPortDialogProps) {
  ...
  const [targetPorts, setTargetPorts] = useState<PortDetails[]>([]);
  const [isLoadingPorts, setIsLoadingPorts] = useState(false);
  const [selectedPortB, setSelectedPortB] = useState<PortDetails | null>(null);
  ...
  useEffect(() => {
    const searchAssets = async () => {
      ...
      const { data, error } = await supabase
        ...
        .neq('id', portA.asset_id) // Exclude the source asset
        .limit(10);
    ...
    searchAssets();
  }, [debouncedSearchTerm, supabase, tenantId, portA.asset_id]);
  ...
  const handleSelectAsset = async (asset: AssetSearchResult) => {
    ...
    const { data, error } = await supabase
      .from('ports')
      .select('*, connections!port_a_id(id), connections!port_b_id(id)')
      .eq('tenant_id', tenantId)
    ...
    if (error) {
      ...
      const freePorts = data.filter(p => p.connections_port_a.length === 0 && p.connections_port_b.length === 0);
      setTargetPorts(freePorts);
    }
  ...
  const memoizedAssetList = useMemo(() => (
    <ScrollArea className="h-64 border rounded-md p-2 bg-input/50">
      {isSearching ? <div className="flex justify-center items-center h-full"><Loader2 className="animate-spin" /></div>
        ...
        ) : <p className="text-center text-sm text-gray-400 pt-4">No se encontraron activos. Escribe para buscar.</p>
      }
    </ScrollArea>
  ...
  return (
    <>
      <DialogHeader>
        ...
        <DialogDescription>
          Busca un activo y selecciona un puerto libre para establecer la conexión.
        </DialogDescription>
      </DialogHeader>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
        ...
        <div>
          <h3 className="font-semibold mb-2 flex items-center"><Server className="mr-2 h-4 w-4 text-sky-400"/>1. Buscar Activo de Destino</h3>
        ...
        <div>
          <h3 className="font-semibold mb-2 flex items-center"><List className="mr-2 h-4 w-4 text-lime-400"/>2. Seleccionar Puerto Libre</h3>
          {selectedAsset ? (
            <>
              <div className="flex items-center justify-between p-2 rounded-md bg-purple-500/10 mb-2">
            ...
            <div className="h-64 border rounded-md p-2 bg-input/50 flex items-center justify-center">
